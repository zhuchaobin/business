...
public class OrderManagementController extends BaseController {
    ...
    public Result<?>  save(ArManagementInVo inVo, String fileUrl) {
    	logger.info("保存订单信息请求报文：ArManagementInVo={}, fileUrl={}", JSON.toJSONString(inVo), JSON.toJSONString(fileUrl));
...
//    	logger.info("长约附件信息长度：{}", inVo.getList().size());
 ...
 /*   @RequestMapping(value = { "queryLnkJrnlInfPage" })
    ...
    public Result<?>  queryLnkJrnlInfPage(ArManagementInVo ArManagementInVo, PageParam pageParam, String arId) {
    	...
    	logger.info("订单信息查询请求参数:{}，分页参数：{}", JSON.toJSONString(ArManagementInVo),JSON.toJSONString(pageParam));
